 ...
 *  PostgreSQL DB Class
 *
 * Original code from {@link http://www.0101app.com vyouzhi (vyouzhi@gmail.com)}
 *
 * @package 
 * @subpackage Database
 * @since 0.71
...
class pgwpdb {
	 ...
	 * to the database.
	 *
	 * @link http://core.trac..org/ticket/3354
	 * @since 2.0.8
	 *       
	 * @param string $dbuser
	 *        	MySQL database user
...
// 			}
	 ...
	 *        	Optional. Whether the table names, e.g. wpdb::$posts, should be updated or not.
	 * @return string WP_Error prefix or WP_Error on error
	 */
	 ...
	 *      
	 * @param string $scope
	 *        	Optional. Can be all, global, ms_global, blog, or old tables. Defaults to all.
	 * @param bool $prefix
	 *        	Optional. Whether to include table prefixes. Default true. If blog
	 *        	prefix is requested, then the custom users and usermeta tables will be mapped.
	 * @param int $blog_id
	 *        	Optional. The blog_id to prefix. Defaults to wpdb::$blogid. Used only when prefix is requested.
...
// </ul>
// <p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://.org/support/"> Support Forums</a>.</p>' ), htmlspecialchars ( $db, ENT_QUOTES ), htmlspecialchars ( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
// 			return;
// 		}
	}
...
// 		else
	 ...
	 * @param string|array $data
	 *        	to escape
	 * @return string array as query safe string
	 */
	 ...
	 *        	The array of variables to substitute into the query's placeholders if being called like
	 *        	{@link http://php.net/vsprintf vsprintf()}, or the first variable to substitute into the query's placeholders if
	 *        	being called like {@link http://php.net/sprintf sprintf()}.
	 * @param mixed $args,...
	 *        	further variables to substitute into the query's placeholders if being called like
	 *        	{@link http://php.net/sprintf sprintf()}.
	 * @return null false string string, null if there is no query, false if there is an error and string
	...
	function db_connect() {
		...
		$conn_string = "host=".$this->dbhost." dbname=".$this->dbname." user=".$this->dbuser." password=".$this->dbpassword." port=2345";
...
" ), htmlspecialchars ( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function query($query) {
		...
		$query = preg_replace('/0000-00-00 00:00:00/i', date('Y-m-d h:i:s', time()), $query);
		//if(substr($query,-1) != ";") $query .= ";";
		
// 				$s = file_get_contents("/tmp/ss");
// 				file_put_contents("/tmp/ss", $s."\r\n\"".$query.";\",");
		...
		if (preg_match ( '/^\s*(create|alter|truncate|drop) /i', $query )) {
			...
			while ( $i < @pg_num_fields ( $this->result ) ) {
				$this->col_info [$i] = $this->pg_fetch_field ( $this->result, $i );
				$i ++;
			}
			$num_rows = 0;
            
			while ( $row = @pg_fetch_object ( $this->result, $num_rows ) ) {    
	 ...
	 *        	Optional. An array of formats to be mapped to each of the value in $data. If string, that format will be used for all of the values in $data.
	 *        	A format is one of '%d', '%f', '%s' (integer, float, string). If omitted, all values in $data will be treated as strings unless otherwise specified in wpdb::$field_types.
	 * @return int false number of rows inserted, or false on error.
	 */
	function insert($table, $data, $format = null) {
		$res = $this->_insert_replace_helper ( $table, $data, $format, 'INSERT' );
	 ...
	 *        	A format is one of '%d', '%f', '%s' (integer, float, string). If omitted, all values in $data will be treated as strings unless otherwise specified in wpdb::$field_types.
	 * @param string $type
	 *        	Optional. What type of operation is this? INSERT or REPLACE. Defaults to INSERT.
	 * @return int false number of rows affected, or false on error.
	 */
	function _insert_replace_helper($table, $data, $format = null, $type = 'INSERT') {
		...
		$sql = "{$type} INTO `$table` (`" . implode ( '`,`', $fields ) . "`) VALUES (" . implode ( ",", $formatted_fields ) . ") returning *";
		$query = $this->prepare ( $sql, $data );
...
// 		file_put_contents("/tmp/ss", $s."\r\n\"".$query.";\",");
	 ...
	 * @param array|string $format
	 *        	Optional. An array of formats to be mapped to each of the values in $data. If string, that format will be used for all of the values in $data.
	 *        	A format is one of '%d', '%f', '%s' (integer, float, string). If omitted, all values in $data will be treated as strings unless otherwise specified in wpdb::$field_types.
	 * @param array|string $where_format
	 *        	Optional. An array of formats to be mapped to each of the values in $where. If string, that format will be used for all of the items in $where. A format is one of '%d', '%f', '%s' (integer, float, string). If omitted, all values in $where will be treated as strings.
	...
	function update($table, $data, $where, $format = null, $where_format = null) {
		...
		$query = $this->prepare ( $sql, array_merge ( array_values ( $data ), array_values ( $where ) ) ) ;
// 		$s = file_get_contents("/tmp/ss");
// 		file_put_contents("/tmp/ss", $s."\r\n\"".$query.";\",");
	 ...
	 *        	SQL query.
	 * @param string $output
	 *        	Optional. one of ARRAY_A | ARRAY_N | OBJECT constants. Return an associative array (column => value, ...),
	 ...
	 *       
	 * @param string|null $query
	 *        	Optional. SQL query. Defaults to previous query.
	 * @param int $x
	 *        	Optional. Column to return. Indexed from 0.
	 ...
	 *        	SQL query.
	 * @param string $output
	 *        	Optional. Any of ARRAY_A | ARRAY_N | OBJECT | OBJECT_K constants. With one of the first three, return an array of rows indexed from 0 by SQL result row number.
	 ...
	 *        	Optional. Type one of name, table, def, max_length, not_null, primary_key, multiple_key, unique_key, numeric, blob, type, unsigned, zerofill
	 * @param int $col_offset
	 *        	Optional. 0: col name. 1: which table the col's in. 2: col's max length. 3: if the col is numeric. 4: col's type
	 ...
	 * @since 2.7.0
	 *       
	 * @return false string on failure, version number on success
	 */
	function db_version() {
